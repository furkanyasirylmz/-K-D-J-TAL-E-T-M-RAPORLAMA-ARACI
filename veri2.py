...
if uploaded_file is not None:
    ...
    try:
        ...
        if 'Etkinlik Kayıt Tarihi' in df.columns:
            ...
            with col1:
                selected_year = st.selectbox("Yıl Seçin", options=years, index=len(years)-1 if len(years) > 0 else 0)
            with col2:
            ...
            if st.button("Rapor Oluştur"):
                ...
                if df_filtered.empty:
                    ...
                    # -- KATILIMCI TAM ADI OLUŞTURMA (Sadece Ad Soyad üzerinden) --
                    # Birincil olarak 'Ad Soyad' kullan, yoksa 'Ad Soyad.1' kullan
                    if 'Ad Soyad' in df_filtered.columns:
                        kat_fullname = df_filtered['Ad Soyad'].astype(str)
                        # Eğer bazı satırlar boşsa ve 'Ad Soyad.1' varsa oradan doldur
                        if 'Ad Soyad.1' in df_filtered.columns:
                            kat_fullname = kat_fullname.where(kat_fullname.str.strip().ne(''), df_filtered['Ad Soyad.1'].astype(str))
                    elif 'Ad Soyad.1' in df_filtered.columns:
                        kat_fullname = df_filtered['Ad Soyad.1'].astype(str)
                    else:
                        # Hiçbiri yoksa net bir hata verelim (akışı bozmadan)
                    ...
                    # Sütunlar güvenlik kontrolü (sıradışı dosyada sütun eksikse patlamasın)
